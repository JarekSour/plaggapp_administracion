...
export class ShowClientComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.clientService.getClientById({ token: localStorage.getItem('token'), user: 'admin', id: this.idClient }).then((response) => {
            if (response['status']) {
        ...
        this.documentoService.getClientDocs({ token: localStorage.getItem('token'), user: 'admin', id: this.idClient }).then((response) => {
    ...
    hideClient() {
        this.clientService.hideClient({ token: localStorage.getItem('token'), user: 'admin', id: this.idClient }).then((response) => {
            if (response['status']) {
    ...
    showClient() {
        this.clientService.showClient({ token: localStorage.getItem('token'), user: 'admin', id: this.idClient }).then((response) => {
            if (response['status']) {
    ...
    deleteDoc(id) {
        this.documentoService.deleteClientDoc({ token: localStorage.getItem('token'), user: 'admin', id: id, id_cliente: this.idClient }).then((response) => {
            if (response['status'] == true) {
