...
export class InformesComponent implements OnInit {
    ...
    public usadosChartLabels: Array<any> = [];
    ...
    public usadosChartData: any[] = [
        { data: [], label: '10 puntos con mayor consumo de cebos' }
    ];
    ...
    btnSearch() {
        ...
        this.adminService.getInforme({ token: localStorage.getItem('token'), user: 'admin', init: init, fini: fini }).then((response) => {
            if (response['status']) {
    ...
    changeCliente2(e) {
        this.usadosChartLabels.length = 0;
        this.clientService.getInformeUso({ token: localStorage.getItem('token'), user: 'admin', id: e.target.value }).then((response) => {
            if (response['status']) {
                ...
                for (let item of json) {
                    this.usadosChartLabels.push(item.nombre)
                    aux.push(item.count);
                    if (aux.length == 10)
                        break;
                }
                let _chartData: Array<any> = new Array(this.usadosChartData.length);
                _chartData[0] = { data: aux, label: this.usadosChartData[0].label };
                this.usadosChartData = _chartData;
            }
        })
    }
